...
class NodeChildrenAsTreeApi(generics.ListAPIView):
    ...
    def assets_amount(self, node):
        ...
        cache_key = ''
        if asset_type == 'dc2':
            cache_key = '_DC2_NODE_ASSETS_AMOUNT_{}'.format(node.key)
    ...
    def get_all_assets(self, node):
        ...
        if asset_type == 'dc2':
            assets = Dc2.objects.filter(*args, **kwargs).distinct()
        return assets
    ...
    def filter_assets(self, queryset):
        ...
        if self.node.is_default_node():
            if asset_type == 'dc2':
                assets = Dc2.objects.filter(Q(nodes__id=self.id) | Q(nodes__isnull=True))
        else:
            if asset_type == 'dc2':
                assets = Dc2.objects.filter(nodes__id=self.id)
...
class NodeAddDc2Api(generics.UpdateAPIView):
    serializer_class = NodeDc2Serializer
    ...
    def perform_update(self, serializer):
        ...
        ret = instance.dc2.add(*tuple(assets))


class NodeRemoveDc2Api(generics.UpdateAPIView):
    serializer_class = NodeDc2Serializer
    queryset = Node.objects.all()
    permission_classes = (IsOrgAdmin,)
    instance = None
    ...
    def perform_update(self, serializer):
        ...
        if instance != Node.root():
            instance.dc2.remove(*tuple(assets))
        else:
            assets = [asset for asset in assets if asset.nodes.count() > 1]
            instance.dc2.remove(*tuple(assets))


class NodeReplaceDc2Api(generics.UpdateAPIView):
    serializer_class = NodeDc2Serializer
    queryset = Node.objects.all()
    permission_classes = (IsOrgAdmin,)
    instance = None
